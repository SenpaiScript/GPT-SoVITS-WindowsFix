...
# Removed DDPStrategy import as it's no longer needed for single GPU
        ...
        if self._should_save_on_train_epoch_end(trainer):
            ...
            if self._every_n_epochs >= 1 and (trainer.current_epoch + 1) % self._every_n_epochs == 0:
                ...
                if self.if_save_every_weights == True:
                    ...
                    # Refactored to access model directly instead of through strategy._lightning_module
                    dictt = pl_module.state_dict()
                    for key in dictt:
                    ...
                    # Simplified rank check since we aren't using DDP
                    my_save(
                        to_save_od,
                        "%s/%s-e%s.ckpt"
                        % (
...
def main(args):
    ...
    # REFACTORED TRAINER: Removed DDPStrategy and Master Addr configs
    trainer: Trainer = Trainer(
        max_epochs=config["train"]["epochs"],
        accelerator="gpu" if torch.cuda.is_available() else "cpu",
        limit_val_batches=0,
        devices=1, # Fixed to 1 device since you're refactoring for single-GPU
        benchmark=False,
        fast_dev_run=False,
        strategy="auto", # PyTorch Lightning will automatically use SingleDeviceStrategy
